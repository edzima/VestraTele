...
foreach ($leadsDaysData as $data) {
	...
	if (!isset($leadsCountSeries[$status])) {
		$statusModel = LeadStatus::getModels()[$status];
		$leadsCountSeries[$status] = [
			'name' => $statusModel->name,
			'data' => [],
		];
		//@todo add chart_group column to LeadStatus
		if ($statusModel->name === 'Umowa') {
			$leadsCountSeries[$status]['group'] = 'Umowa';
		} else {
			$leadsCountSeries[$status]['group'] = 'Reszta';
		}
		$leadsDaysData['colors'][] = $leadStatusColor->getStatusColor($statusModel);
	}
	//datetime
//	$leadsCountSeries[$status]['data'][] = [
//		$data['date'],
//		(int) $data['count'],
//	];
	//category
	$leadsCountSeries[$status]['data'][] = (int) $data['count'];
			...
			<div class="col-md-12" style="height:50vh">
				<?= ChartsWidget::widget([
					...
					'options' => [
						...
						'xaxis' => [
							'type' => 'datetime',
							'categories' => array_values($leadsDaysData['labels']),
						],
					],
				]) ?>
			</div>
