...
class CalculationFormTest extends PayFormTest {
	...
	protected function grabIssue($index = 0): Issue {
		return $this->tester->grabFixture(IssueFixtureHelper::ISSUE, $index);
	}

	protected function createForm(array $config = []): CalculationForm {
		$issue = ArrayHelper::remove($config, 'issue', $this->grabIssue());
		$ownerId = ArrayHelper::remove($config, 'ownerId', static::AGENT_ID);
