...
do
    case $opt in
        "Development ====> YII SERVE(frontend) + YII SERVE(backend) + NGNX(reverse proxy) + MYSQL")
            ...
            toMerge=("environments/.env.common" "environments/.env.docker" "environments/.env.development");
            mergeEnvs "${toMerge[@]}"
        ...
        "Production ====> APACHE(frontend)+ APACHE(backend) + NGNX(reverse proxy) + MYSQL")
            ...
            toMerge=("environments/.env.common" "environments/.env.docker" "environments/.env.production");
            mergeEnvs "${toMerge[@]}"
        ...
        "Tests =====> PHP(YII) + MYSQL")
            ...
            toMerge=("environments/.env.common" "environments/.env.docker" "environments/.env.test");
            mergeEnvs "${toMerge[@]}"
